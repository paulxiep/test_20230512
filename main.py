...
if order_file:
    ...
    q8 = order[['Company', 'Supplier', 'PartNum', 'POAmount_THB']].groupby(['Company', 'Supplier', 'PartNum']).sum().reset_index()
    ...
    if contract_file:
        ...
        with tabs[4]:
            ...
            st.markdown(f'PartNum with unexpired contracts are :green[{reduce(lambda x, y: x + " and " + y, q5[pd.to_datetime(q5["ExpiredDate"]) > datetime.datetime(2021, 5, 21, 7)]["PartNum"].unique().tolist())}].')
        ...
        with tabs[5]:
            ...
            st.markdown(f"Of this, :green[{len(q6[q6['UnitPrice_x'].notna() & q6['UnitPrice_y'].notna()].index)}] POs exist in the contracts table.")
            st.markdown(f":green[{len(q6[q6['UnitPrice_x'].notna() & q6['UnitPrice_y'].isna()].index)}] ContractNo-BuyerPartNum has no corresponding PO in order table.")
            st.markdown(f":green[{len(q6[q6['UnitPrice_x'].isna() & q6['UnitPrice_y'].notna()].index)}] POs have null contract entries.")

        with tabs[6]:
            st.subheader('Q7')
            st.text('จากข้อ 6 รายการสินค้าที่ราคาไม่ตรงกับสัญญาคิดเป็นมูลค่าเท่าใด')
            st.markdown(f"Out of total value of :green[~{int(order['POAmount_THB'].sum()//1000000)}] M of POAmount of POs in order table,")
            st.markdown(f":green[~{int(q7[q7['UnitPrice_x']!=q7['UnitPrice_y']]['POAmount_THB'].sum()//1000000)}] M has UnitPrices that differ between contract and actual orders")
